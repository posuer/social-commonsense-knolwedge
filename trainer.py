...
class Trainer:
    ...
    best_acc: Optional[float] = -1
    eval_acc_list: Optional[dict] = dict()
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    if self.args.evaluate_during_training and self.global_step % self.args.eval_steps == 0:
                        ...
                        if eval_metrics['eval_acc'] > self.best_acc:
                            last_best_step = self.best_step
                    ...
                    if self.args.save_steps > 0 and self.global_step % self.args.save_steps == 0 and self.best_step == self.global_step:
                        ...
                        self.delete_model(last_best_step)
    ...
    def delete_model(self, last_best_step):
        deleting_model_path = os.path.join(self.args.output_dir, f"{PREFIX_CHECKPOINT_DIR}-{last_best_step}")
        if os.path.exists(deleting_model_path):
            for root, subdirs, files in os.walk(deleting_model_path):
                for file in files:
                    os.remove(os.path.join(root, file))
            os.rmdir(deleting_model_path)
